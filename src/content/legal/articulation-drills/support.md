...
This usually happens after reinstalling the app or moving to a new device. Tap
**Restore purchases** in Settings, signed in with the same Apple Account used for
the original purchase, and the sets will unlock. If they don't, email us with the
date of purchase and we'll help.
...
No. Every picture and recording is stored inside the app, so it works fully
offline. A connection is needed only to make or restore a purchase, which Apple
handles.
...
Yes. Every word is spoken by Christina Sibley, a professional child actress.
Children match a peer's voice more readily than an adult's or a synthesised one.
